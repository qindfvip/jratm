<table id="${id}" width="auto" height="auto"
			data-options="
					...
					<%if(isEmpty(url)){%>
					<%// 默认模式%>
					url : '${ADMIN_PATH}/grid/query/${obj.code}',
					<%}else{%>
					<%// 自定义URL%>
					url : '${url}',
					<%}%>
	...
	$(function(){
					...
					<%if(item.isShow!true){%>
                        ...
                        <%if(isTrue(obj.isCellEdit) && isTrue(item.isEdit) && !isTrue(item.isAuto)){%>
                            editor:{type:'${item.editor}',options:{
                                url: '${ADMIN_PATH}/widget/comboJson/${item.objectCode}-${item.en}',
                                valueField : 'ID',
                                textField : 'CN'
                            }},
                        <%}%>
		...
		function createRowMenu(){
			...
			rowMenu.menu('appendItem', {
				...
				onclick: function(){
					window.location.href='${ADMIN_PATH}/grid/export/${obj.code}';
				}
			});
			<%if(isTrue(obj.isCellEdit)){%>
			...
			rowMenu.menu('appendItem', {
				...
				onclick: function(){
                    ...
                    $.mypost('${ADMIN_PATH}/grid/add/${obj.code}',{rows : json1},
                    ...
                    $.mypost('${ADMIN_PATH}/grid/delete/${obj.code}',{rows : json2},
                    function(result , status){
                        if (!result.success) {
                            isOk = false;
                        }
                    ...
                    $.mypost('${ADMIN_PATH}/grid/update/${obj.code}',{rows : json3},
                    function(result , status){
                        if (!result.success) {
                            isOk = false;
                        }
